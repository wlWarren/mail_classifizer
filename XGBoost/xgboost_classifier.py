...
def train_model(train_data,train_label,test_data,test_label):
    ...
    print 'trainning time:%f' % (time.time() - start_time)
    # 存储模型
    joblib.dump(model,'model/XGBoost_model_60w.pkl')
...
if __name__ == '__main__':
    ...
    train_data = np.array(tfidf.transform(train_data).todense())
    test_data = np.array(tfidf.transform(test_data).todense())
